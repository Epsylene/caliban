...
pub unsafe fn create_texture_image(
    ...
    // a transfer operation (because of the blit operation
    // needed to generate the mipmaps), and stored on the GPU.
...
unsafe fn generate_mipmaps(
    ...
    // Mipmaps (from MIP mapping, itself from the Latin "multum
    // in parvo", "much in a little") are precomputed,
    // optimized sequences of textures that are used to
    // accelerate texture sampling and reduce aliasing
    // artifacts. They are generated by downsampling the
    // original texture into smaller images, each half the size
    // of the previous one, until the last image is has a size
    // of one pixel. This way, different mip levels can be
    // combined to sample the texture at different distances,
    // reducing the number of texels to sample and avoiding
    // Moiré patterns at large distances.

    let command_buffer = begin_single_command_batch(device, data)?;

    // We will begin by checking that the device supports
    // linear filtering, which is needed later for
    // interpolation between mip levels.
    if !instance
